...
if ! $skip_scoring ; then
  [ ! -x steps/scoring/score_kaldi_wer.sh ] && \
    echo "$0: Not scoring because steps/scoring/score_kaldi_wer.sh does not exist or not executable." && exit 1;
  steps/scoring/score_kaldi_wer.sh --cmd "$cmd" $scoring_opts $data $graphdir $dir ||
