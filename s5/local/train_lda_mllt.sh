...
# Create graph:
#./utils/mkgraph.sh data/lang exp/tri1a exp/tri1a/graph
###./utils/mkgraph.sh data/output exp/tri1a exp/tri1a/graph

# Decode:
#./steps/decode.sh --nj $NJDECODE --cmd "$train_cmd" exp/tri1a/graph data/test_data exp/tri1a/decode_test
#./steps/decode.sh --nj 4 --cmd "$train_cmd" exp/tri1a/graph "$TESTDIR" exp/tri1a/decode_test

# Continue training the model:
./steps/align_si.sh --nj $NJ --cmd "$train_cmd" "$TRAINDIR" data/output exp/tri1a exp/tri1a_ali

./steps/train_deltas.sh --cmd "$train_cmd" 2500 15000 "$TRAINDIR" data/output exp/tri1a_ali exp/tri2a

./steps/align_si.sh --nj $NJ --cmd "$train_cmd" --use-graphs true "$TRAINDIR" data/output exp/tri2a exp/tri2a_ali

# Train LDA-MLLT triphones
./steps/train_lda_mllt.sh --cmd "$train_cmd" 3500 20000 "$TRAINDIR" data/output exp/tri2a_ali exp/tri3a

# Create graph:
./utils/mkgraph.sh data/output exp/tri3a exp/tri3a/graph
# Decode:
